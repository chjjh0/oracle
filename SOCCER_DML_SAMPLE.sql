...
--VER2
SELECT
    (SELECT T2.TEAM_NAME FROM TEAM T2 WHERE T2.TEAM_ID LIKE A.팀명) 팀명,
    A.평균키
FROM
(SELECT
    T.TEAM_ID 팀명,
...
WHERE P.POSITION LIKE 'GK' GROUP BY T.TEAM_ID ORDER BY 평균키 DESC) A
WHERE ROWNUM = 1
;
...
--ver2
SELECT
    (SELECT T2.TEAM_NAME FROM TEAM T2 WHERE T2.TEAM_ID LIKE T.TEAM_ID) 팀명,
    ROUND(AVG(P.HEIGHT),1) 평균키
FROM PLAYER P
    JOIN TEAM T
        ON P.TEAM_ID LIKE T.TEAM_ID
GROUP BY T.TEAM_ID
HAVING ROUND(AVG(P.HEIGHT),1) < (SELECT ROUND(AVG(P2.HEIGHT),1)
                        FROM PLAYER P2 JOIN TEAM T3 ON P2.TEAM_ID LIKE T3.TEAM_ID 
                        WHERE T3.TEAM_NAME LIKE '삼성블루윙즈')
;
